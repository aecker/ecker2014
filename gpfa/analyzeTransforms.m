...
nUnits = count(nc.GpfaParams * nc.GpfaModelSet * nc.GpfaUnits & key) / nTrans / 2;
...
for z = 1 : 2
    ...
    for i = 1 : nTrans
        ...
        hold on
        [m, binsc] = makeBinned(fr(:, i, z), ve(:, i, z), bins, key(1).fun, 'include');
        plot(binsc, m, '*-r')
        axis tight
...
figure(100 * key(1).subject_id + 10 * key(1).by_trial + key(1).control), clf
resize(gcf, [330 420])
subplot(2, 1, 1), hold all
colors = colormap(lines);
for i = 1 : nTrans
    for z = 1 : 2
        bar((i - 1) * 2 + z, key(1).fun(ve(:, i, z)), 0.5, 'facecolor', (1 - (z - 1) / 2) * colors(i, :) + (z - 1) / 2)
    end
end
...
for i = 1 : nTrans
    for z = 1 : 2
        m = makeBinned(fr(:, i, z), ve(:, i, z), bins, key(1).fun, 'include');
...
for transform = fetch(nc.DataTransforms)'
    for z = [0 1]
        modelKey = dj.struct.join(struct('zscore', z), key);
        modelKey = dj.struct.join(modelKey, transform);
        rel = nc.GpfaParams * nc.GpfaModelSet * nc.GpfaCovExpl & modelKey;
        vea(:, z + 1, tr) = fetchn(rel, '1 - avg_var_unexpl_test -> v');
    end
    tr = tr + 1;
end
