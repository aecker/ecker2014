%{
nc.GpfaSpontResidCorrSet (computed) # Residual correlations for GPFA model

-> nc.GpfaSpontSet
---
%}

classdef GpfaSpontResidCorrSet < dj.Relvar & dj.AutoPopulate
    properties(Constant)
        table = dj.Table('nc.GpfaSpontResidCorrSet');
        popRel = nc.GpfaParams * nc.GpfaSpontSet & 'kfold_cv > 1';
    end

    methods (Access = protected)
        function makeTuples(self, key)
            self.insert(key);
            Yt = fetch1(nc.GpfaSpontSet & key, 'transformed_data');
            T = size(Yt, 2);
            ...
            for p = 0 : par.max_latent_dim
                for int = 1 : T
                    ...
                    for k = 1 : par.kfold_cv
                        ...
                        modelKey.cv_run = k;
                        [train, test, model] = fetch1(nc.GpfaSpont & modelKey, 'train_set', 'test_set', 'model');
                        model = GPFA(model);
                    ...
                    [ii, jj, pairs] = fetchn(nc.GpfaSpontPairs & key, 'index_i', 'index_j');
                    tuple = key;
                    tuple.latent_dim = p;
                    tuple.int_bins = int;
                    for iPair = 1 : numel(pairs)
                        ...
                        insert(nc.GpfaSpontResidCorr, tuple);
                    end

                    % insert variance explained per cell
                    unitIds = fetchn(nc.GpfaSpontUnits & key, 'unit_id', 'ORDER BY unit_id');
